...
void ntrip_client_add_mountpoint(const char* mountpoint)
{
    ...
    {
        strncpy(g_ntrip_mountpoints[g_ntrip_mountpoint_count], mountpoint, NTRIP_MOUNTPOINT_LEN_MAX - 1);
        g_ntrip_mountpoints[g_ntrip_mountpoint_count][NTRIP_MOUNTPOINT_LEN_MAX - 1] = '\0';
...
{
    return g_ntrip_mountpoint_count;
}

const char* ntrip_client_get_mountpoint(int index)
{
    if (index < 0 || index >= g_ntrip_mountpoint_count)
    {
        return NULL;
    }
    return g_ntrip_mountpoints[index];
}

static void ntrip_get_mountpoints_task(void* pvParameters)
{
...
void ntrip_client_get_mountpoints(const char* host, uint16_t port, const char* username, const char* password)
{
    ...
    {
        ESP_LOGE(TAG, "Invalid parameters for ntrip_client_get_mountpoints");
        return;
    }
    ...
    xTaskCreate(ntrip_get_mountpoints_task, "ntrip_get_mp", 4096, args, 5, NULL);
}
...
static void ntrip_stream_task(void* pvParameters)
{
    ...
    status_set(STT_NTRIP_CLIENT_STATUS, NTRIP_CONNECTING);
    ...
    if (client == NULL)
    {
        ...
        free(args);
        status_set(STT_NTRIP_CLIENT_STATUS, NTRIP_DISCONNECTED);
    ...
    status_set(STT_NTRIP_CLIENT_STATUS, NTRIP_CONNECTED);
