...
#define UBXSYNC1 0xB5 /* ubx sync code 1 */
#define UBXSYNC2 0x62 /* ubx sync code 2 */
#define UBXCFG   0x06 /* ubx class cfg */
...
uint32_t ubx_gen_cmd(const char* msg, uint8_t* buff)
{
    ...
    /* Working buffer for parsing input string (strtok modifies it) */
    static char mbuff[128];

    /* Validate input length before copying */
    size_t msg_len = strlen(msg);
    if (msg_len >= sizeof(mbuff))
    {
        return 0;
    }
    memcpy(mbuff, msg, msg_len + 1);

    /* Parse input string into arguments */
    char* args[32];
    int32_t narg = 0;

    for (char* p = strtok(mbuff, " "); p && narg < 32; p = strtok(NULL, " "))
    {
        args[narg++] = p;
    }

    /* Validate command format: must start with "CFG-" */
    if (narg < 1 || strncmp(args[0], "CFG-", 4))
    {
        return 0;
    }

    /* Find command index in command table */
    int32_t i;
    for (i = 0; *cmd[i]; i++)
    {
    ...
    /* Build UBX message header */
    *q++ = UBXSYNC1;
    *q++ = UBXSYNC2;
    *q++ = UBXCFG;
    *q++ = id[i];
    q += 2; /* Skip length field, will be filled later */

    /* Check if this is a VALSET command */
    bool isvalset = (strcmp(cmd[i], "VALSET") == 0);
    ...
    /* Encode command parameters based on parameter type table */
    int32_t j;
    ...
    if (isvalset)
    {
        ...
        /* Find VALSET config data index in vcmd table */
        int32_t k;
        for (k = 0; *vcmd[k]; k++)
        {
        ...
        /* Encode VALSET key ID */
        setU4(q, (unsigned long)vid[k]);
        q += 4;

        /* Encode VALSET value based on type */
        switch (vprm[k])
        {
            case FU1:
    ...
    /* Calculate total message length and finalize */
    int32_t n = (int32_t)(q - buff) + 2;
    setU2(buff + 4, (uint16_t)(n - 8));
    set_checksum(buff, n);
    return n;
}
...
{
    char* buffer = calloc(n * 3 + 1, sizeof(char));
    char* p = buffer;
...
void validate_ubx_gen_cmd(void)
{
    ...
    memset(msg, 0, 64);
    sprintf(msg, "CFG-VALSET 0 1 0 0 CFG-TMODE-LAT %li", (int32_t)(lat * pow(10, 7)));
    ...
    /*
    double lon = 105.7684480;
    memset(msg, 0, 64);
    sprintf(msg, "CFG-VALSET 0 1 0 0 CFG-TMODE-LON %li", (int32_t)(lon * pow(10, 7)));
    */
    ...
    /*
    double height = -1.0000000;
    memset(msg, 0, 64);
    sprintf(msg, "CFG-VALSET 0 1 0 0 CFG-TMODE-HEIGHT %li", (int32_t)(height * pow(10, 2)));
    */
